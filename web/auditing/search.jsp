...
<spring:bind path="cmd.*">
	...
	<div>
		...
		<form id="fields" name="fields" method="POST"
			action="<c:url value="/auditing/search.html"/>"><input type="hidden" size=20
			maxlength=40 name="option" value="${option}" />

		<table border="0" cellspacing="1" cellpadding="0" id="ep" style="margin-right:40px" width="80%">

			<tr>
				<td width="30%">
				<abc:i18n property="auditing.caption.user"/>
...
<!--					</c:forEach>-->
<!--				</select>-->
						<spring:bind path="cmd.user">
								<select name="${status.expression}" id="${status.expression}" >
								<option value="" ><fmt:message key="auditing.caption.allUsers" /></option>
									<c:forEach items="${users}" var="user">
										<option value="${user.id}" ${cmd.user == user.id ? ' selected' : ''}>${user.username}</option>
   									</c:forEach>
								</select>
							</spring:bind>
				</td>
				<td width="30%">
				<abc:i18n property="auditing.caption.action"/>
				<fmt:message key="auditing.caption.action" /></td>
				<td width="20%"><spring:bind path="cmd.action"><select name="${status.expression}" id="${status.expression}">
					<option value=""><fmt:message key="auditing.caption.allActions"/></option>
					<option value="create" ${cmd.action == 'create' ? ' selected' : ''}><fmt:message key="auditing.caption.create" /></option>
					<option value="update"${cmd.action == 'update' ? ' selected' : ''}><fmt:message key="auditing.caption.update" /></option>
			...
			<tr>
				...
				<td width="20%">
<!--				<input type="text" name="dateFrom" va>-->
				<spring:bind path="cmd.fromDate">
...
<!--				<input type="text" name="dateTo">-->
			...
			<tr>
				...
				<td>
				<spring:bind path="cmd.className" >
								<select name="${status.expression}" id="${status.expression}" onchange="Check();" id="selectClass">
								<option value="" ><fmt:message key="auditing.caption.allClasses" /></option>
									<c:forEach items="${classNames}" var="classNames">
										<option value="${classNames.entityClass}" ${cmd.className == classNames.entityClass ? ' selected' : ''}>${classNames.name}</option>
   									</c:forEach>
								</select>
							</spring:bind>
				
<!--				<select name="selectClass" onchange="Check();" id="selectClass">-->
...
<!--				</select>-->
			...
			<tr id="btn">
				...
				<input type="submit" name="search"
					value="<fmt:message key="commons.button.search"/>" class="button" />&nbsp;&nbsp;&nbsp;
