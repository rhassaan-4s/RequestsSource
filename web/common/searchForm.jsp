...
<input type="hidden" name="canSeeAllStore" id="canSeeAllStore"
	value="${canSeeAllStore}" />
				...
				<table width="90%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						<td colspan="2"><form:form method="POST"
								...
								action="/Requests/common/searchForm.html">
								<table rules="all" align="center" width="70%" class="sofT">
									...
									<tr>
										...
										<c:if test="${table!='dist_names'}">
											<td class="formBodControl"><input type="text" size="15"
												name="searchCommandId" id="name" value="${searchCommandId}"
												onkeyup="enableFilter('name');" value="${searchCommandId}" /></td>
										</c:if>
										<c:if test="${table=='dist_names'}">
											<td class="formBodControl"><input type="text" size="15"
												name="searchCommandId" id="name"
												onkeyup="enableFilter('name');" value="${searchCommandId}" /></td>
										</c:if>
										<td nowrap class="formBod"><abc:i18n
												property="${commons.caption.matchMode}" /> <fmt:message
												key="commons.caption.matchMode" /></td>
										<td class="formBodControl"><select name="match1"
											id="match1">
									...
									<c:if test="${table=='dist_names'}">
										<tr>
											<td nowrap class="formBod"><abc:i18n
													property="commons.caption.Code" /> <fmt:message>commons.caption.Code</fmt:message></td>
											<td class="formBodControl"><input type="text" size="15"
												name="searchCommandName" id="code"
												onkeyup="enableFilter('code');" value="${searchCommandName}" /></td>
											<td nowrap class="formBod"><abc:i18n
													property="${commons.caption.matchMode}" /> <fmt:message
													key="commons.caption.matchMode" /></td>
					...
					<tr>
					 ...
					 -->
									...
									<c:if
										test="${canSeeAllStore=='true' && table =='STORE_STORES_LIST'}">
										<tr>
									...
									<c:if test="${transTableFlag=='true'}">
										<tr>
											...
											<td nowrap class="helpTitle"><c:if
													test="${fromDate != null}">
													<input type="text" class="calendar" id="fromDate"
														name="fromDate" value="${fromDate}">
												</c:if> <c:if test="${fromDate == null}">
													<input type="text" class="calendar" id="fromDate"
														name="fromDate" value="${dateNow}">
												</c:if></td>
											<td nowrap class="helpTitle"><abc:i18n
											...
											<td nowrap class="helpTitle"><c:if
													test="${toDate != null}">
													<input type="text" class="calendar" id="toDate"
														name="toDate" value="${toDate}">
												</c:if> <c:if test="${toDate == null}">
													<input type="text" class="calendar" id="toDate"
														name="toDate" value="${dateNow}">
												</c:if></td>
										</tr>
						...
						<c:if test="${canSeeAllStore == false}">
							...
							<abc:autocomplete inputId="toCode"
								...
								paramString="${paramStringTo}@@${branchId}"/>
									...
									<input type="hidden" name="storeTransDefId"
										value="${storeTransDefId}" />
									...
									<input type="hidden" name="branchForValidRoom"
										value="${branchForValidRoom}" />
								</table>
								</form:form>
								<br />
								...
								<table width=100% border=0 cellpadding=0 cellspacing=0
									rules="all" align="center" class="sortable">
									<tr id="head_1_ep">
										<c:if
											test="${(table=='view_store_dep_trans' || table =='store_c_trns_m') && transMapList[0].trns_code.storeTrnsDefSpec.is_contract == '0' && transMapList[0].trns_code.storeTrnsDefSpec.is_reservation == '0'}">
											<td nowrap class="helpTitle">&nbsp; <fmt:message
													key="stores.caption.transNumber" />&nbsp;
											</td>
											<td nowrap class="helpTitle">&nbsp; <fmt:message
													key="stores.caption.refrigerator" />&nbsp;
											</td>
											<td nowrap class="helpTitle">&nbsp; <fmt:message
													key="stores.caption.fromDestination" />&nbsp;
											</td>
											<td nowrap class="helpTitle">&nbsp; <fmt:message
													key="stores.caption.toDestination" />&nbsp;
											</td>
										</c:if>
										<c:if
											test="${table =='store_c_trns_m' && transMapList[0].trns_code.storeTrnsDefSpec.is_contract == '1' || transMapList[0].trns_code.storeTrnsDefSpec.is_reservation == '1'}">
											<td nowrap class="helpTitle">&nbsp; <fmt:message
													key="stores.caption.transNumber" />&nbsp;
											</td>
											<td nowrap class="helpTitle">&nbsp; <fmt:message
													key="commons.caption.transDate" />&nbsp;
											</td>
											<td nowrap class="helpTitle">&nbsp; <fmt:message
													key="stores.caption.refrigerator" />&nbsp;
											</td>
											<td nowrap class="helpTitle">&nbsp; <fmt:message
													key="stores.caption.destCustomer" />&nbsp;
											</td>
										</c:if>

										<c:if
											test="${table !='store_c_trns_m' && table!='view_store_dep_trans'}">
											<td class="helpTitle" nowrap align="center">&nbsp;<abc:i18n
													property="${firstKey}" /> <fmt:message>${firstKey}</fmt:message>&nbsp;
											</td>

											<c:if test="${secondKey != null && secondKey != ''}">
												<td class="helpTitle" nowrap align="center">&nbsp;<abc:i18n
														property="${secondKey}" /> <fmt:message>${secondKey}</fmt:message>&nbsp;
												</td>
											</c:if>
										</c:if>
									...
									<c:if
										...
										<c:forEach items="${transMapList}" var="aResult">
											...
											<tr>
												...
												<td nowrap class="helpBod">&nbsp;
													${aResult.branch.descr}</td>
												<c:if test="${aResult.extdestfrom==false}">
													<td nowrap class="helpBod">&nbsp;
														${aResult.fromDestination.name_ar}</td>
												</c:if>
												<c:if test="${aResult.extdestfrom==true}">
													<td nowrap class="helpBod">&nbsp;
														${aResult.extFromCode}</td>
												</c:if>
												<c:if test="${aResult.extdestto==false}">
													<td nowrap class="helpBod">&nbsp;
														${aResult.toDestination.name_ar}</td>
												</c:if>
									...
									<c:if
										...
										<c:forEach items="${transMapList}" var="aResult">
											...
											<tr>
												...
												<td nowrap class="helpBod">&nbsp; <_4s_:formatMiladiDate
														value="${aResult.trns_date}" /></td>
												<td nowrap class="helpBod">&nbsp;
													${aResult.branch.code} - ${aResult.branch.descr}</td>
												<td nowrap class="helpBod">&nbsp;
													${aResult.fromDestination.name_ar}</td>
											</tr>
