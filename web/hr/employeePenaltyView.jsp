...
function deleteMyRow(deleteId,employeeId)
{
	...
	else
	{
		 ...
		 var URL='employeePenaltyView.html?formType=${formType}&deleteId='+deleteId+'&employeeId='+employeeId;
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px ">
  ...
  <form method="post" action="<c:url value="employeePenaltyView.html?add=false"/>">
  <c:if test="${formType == 'transaction'}">
		...
		<b><font color="red"><fmt:message key="hr.errors.noEmployeeSelected"/> </font></b>
			...
			<tr>
				<td colspan="8" class="helpTitle"><abc:i18n
					property="hr.header.employeePenaltyView" /><fmt:message
					key="hr.header.employeePenaltyView" /></td>
			</tr>
			...
			<tr>
			    ...
			    <td class="helpHed" nowrap="nowrap"><abc:i18n
					property="hr.caption.penalty" /> <fmt:message
					key="hr.caption.penalty" /></td>
					<td class="helpHed" nowrap="nowrap"><abc:i18n
					property="hr.caption.penaltyDate" /> <fmt:message
					key="hr.caption.penaltyDate" /></td>
				<td class="helpHed" nowrap="nowrap"> </td>
				<td class="helpHed" nowrap="nowrap"> </td>
				
				
			</tr>
						   <c:forEach items="${empPenaltyList}" var="record">
							   <tr height=20 bgcolor="#F8F8F8">
								  ...
								  <td class="helpBod" nowrap>
								    ...
								    <td class="helpBod" nowrap>
							   <_4s_:formatMiladiDate value="${record.penaltyDate }"/>      
								  ...
								  <td class="helpBod" nowrap><abc:i18n
									...
									href="employeePenaltyForm.html?empPenaltyId=${record.id}&employeeId=${record.employee.id }&formType=${formType }"><fmt:message
		...
		<br>
		    ...
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeePenaltyForm.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&employeeId=${employeeId}&add=true'"></input>
		    </c:if>
		    
		    <c:if test="${formType=='transaction'}">
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeePenaltyView.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&add=true'"></input>
		    </c:if>	 	 
