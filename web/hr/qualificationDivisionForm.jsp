...
<tr>
		...
		<table cellspacing="0" cellpadding="0" border="0" align="right"
			...
			<table cellspacing="0" cellpadding="0" border="0" align="right">
					...
					<c:set var="i" value="qualificationDivTree" />
									...
									<tree:nodeMatch node="tree.node" hasChildren="true"
										...
										<c:choose>
											<c:when test="${result.id != null}">
												<a
													href="qualificationDivisionTree.html?expand=<tree:nodeId node="tree.node"/>&qualificationDivisionId=${result.id}"><img
										...
										<c:choose>
											...
											<c:otherwise>
												<a
													href="qualificationDivisionTree.html?collapse=<tree:nodeId node="tree.node"/>"><img
													src="/Requests/web/gl/images/ar_expandedMidNode.gif" border="0"></a>
											</c:otherwise>
										</c:choose>
									</tree:nodeMatch>
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px" align="left" >
   <form id="qualificationDivisionForm" name="qualificationDivisionForm" method="POST"	action="<c:url value="/hr/qualificationDivisionForm.html"/>">
		  ...
		  <c:if test="${copyId!=null && copyId!=''}">
					...
					<td class="formBodControl">
						...
						<option value=""><fmt:message key="commons.caption.select" /></option>
							<c:forEach items="${qualDivList}" var="qualDiv">
								<option value="${qualDiv.id}" ${qualDiv.id == trainingExam.trainingProgram.id ?' selected' : ''}>${qualDiv.ardesc}</option>
							</c:forEach>
						</select>
					...
					<td class="formBodControl">
						<select multiple size="6" name="leafsList" id="leafsList" >
							<c:forEach items="${leafs}" var="leaf">
								...
								<c:forEach items="${qualificationDivision.childs}" var="child">
