...
function deleteMyRow(deleteId,employeeId)
{
	...
	else
	{
		 ...
		 var URL='employeeSponsorsView.html?formType=${formType}&deleteId='+deleteId+'&employeeId='+employeeId;
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px ">
  ...
  <form method="post" action="<c:url value="employeeSponsorsView.html?add=false"/>">
  <c:if test="${formType == 'transaction'}">
		...
		<b><font color="red"><fmt:message key="hr.errors.noEmployeeSelected"/> </font></b>
			...
			<tr>
				<td colspan="8" class="helpTitle"><abc:i18n
					property="hr.header.employeeSponsorsView" /><fmt:message
					key="hr.header.employeeSponsorsView" /></td>
			</tr>
			...
			<tr>
			    ...
			    <td class="helpHed" nowrap="nowrap"><abc:i18n
					property="hr.caption.sponsor" /> <fmt:message
					key="hr.caption.sponsor" /></td>
					<td class="helpHed" nowrap="nowrap"><abc:i18n
				...
				<td class="helpHed" nowrap="nowrap"> </td>
						   ...
						   <c:forEach items="${empSponsorsList}" var="record">
							   <tr height=20 bgcolor="#F8F8F8">
								  ...
								  <td class="helpBod" nowrap>
								    ...
								    <td class="helpBod" nowrap>
							   <_4s_:formatMiladiDate value="${record.sponsorStartDate }"/>      
								  ...
								  <td class="helpBod" nowrap><abc:i18n
									...
									href="employeeSponsorForm.html?empSponsorId=${record.id}&employeeId=${record.employee.id }&formType=${formType }"><fmt:message
		...
		<br>
		    ...
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeSponsorForm.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&employeeId=${employeeId}&add=true'"></input>
		    </c:if>
		    
		    <c:if test="${formType=='transaction'}">
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeSponsorsView.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&add=true'"></input>
		    </c:if>	 	 
