...
if (settings.getWithoutSalaryVacEnabled().booleanValue() == false) {%>
	...
	<%}%>
		...
		function getVacCredit() {
			...
			var URL = 'loginUsersRequestsForm.html?vacation=' + vacId
					+ '&fromDate=' + from_date + '&request_id=' + request_id;
		...
		function calculateDiff() {
			...
			if (dateFrom != '' && dateTo != '') {
				...
				if (diffMonths != 0) {
					...
					document.getElementById('withdrawDays').value = Math
							.abs(days) + 1;
	...
	<%}%>
		...
		function success(pos) {
			...
			document.getElementById("latitude").setAttribute("value",
					crd.latitude);
			document.getElementById("accuracy").setAttribute("value",
					crd.accuracy);
			...
			<td><form:form method="POST"
					...
					<table border=0 cellspacing=1 cellpadding=0 id="ep"
						...
						<tr class="showClass">
							...
							<td class="formBodControl"><input type="hidden"
								name="empCode" id="empCode" value="${model.employeeCode}" />
								${model.employeeCode}</td>
									...
									<select name="request_id" id="${status.expression}"
										...
										<c:forEach items="${model.requestTypeList}" var="request">
											<option value="${request.id}"
												${request.id == loginUsersRequests.request_id.id ?' selected' : ''}
												onclick="showVacationTypes()">${request.description}</option>
										</c:forEach>
									</select>
								</spring:bind></td>
						</tr>
						...
						<c:if
							...
							<tr id="annualVacationTypes" class="hideClass">
								...
								<c:if test="${settings.annualVacBalDaysEnabled==true}">
									...
									<td class="formBodControl"><input type="text" size="10"
										readonly="readonly" name="vacCredit" id="vacCredit"
										value="${model.vacCredit}" /></td>
								</c:if>
							...
							<tr id="vacationDates" class="hideClass">
								...
								<td class="formBodControl"><input type="text"
									name="withdrawDays" id="withdrawDays" onfocus="calculateDiff()" />
								</td>
							</tr>
							...
							<tr id="leavePermission" class="hideClass">
								...
								<td class="formBodControl"><spring:bind
										path="loginUsersRequests.leave_effect">
										<select name="${status.expression}" id="${status.expression}">
											...
											<option value="2" ${status.value == "2" ? ' selected':''}><fmt:message
													key="requestsApproval.caption.early" /></option>
										</select>
									</spring:bind></td>
							</tr>
							...
							<tr id="vacWithoutSal" class="hideClass">
								...
								<td nowrap class="formReq"><abc:i18n
									...
									<fmt:message
										key="requestsApproval.requestsApprovalForm.reqPeriod" /></td>
								<td class="formBodControl"><select name="withoutSalPeriod"
									id="withoutSalPeriod">
										<option value=""><fmt:message
												key="commons.caption.select" /></option>
										<%
										...
										%>
										<option value="quar"
											${status.value == "quar" ? ' selected':''}><fmt:message
												key="requestsApproval.caption.quarDay" /></option>
										<%
										}
										%>
										<option value="half"
								...
								%>
								<td nowrap class="formReq"><abc:i18n
										property="requestsApproval.caption.leaveTime" /> <fmt:message
										key="requestsApproval.caption.leaveTime" /></td>

								<td class="formBodControl"><select name="leaveTime"
									id="leaveTime">
										<option value=""><fmt:message
												key="commons.caption.select" /></option>
										<option value="start"
											${status.value == "start" ? ' selected':''}><fmt:message
												key="requestsApproval.caption.leaveTimeStart" /></option>
										<option value="end" ${status.value == "end" ? ' selected':''}><fmt:message
												key="requestsApproval.caption.leaveTimeEnd" /></option>
								</select></td>
								<%
								}
								%>
								...
								<td class="formBodControl"><input type="text"
									name="withdraw" id="withdraw" onfocus="calculateDiff2()"
									value="" /></td>
								<%
								}
								%>
						...
						<c:if
								...
								<c:if test="${loginUsersRequests.request_id.id==1}">
									...
									<c:if test="${loginUsersRequests.vacation.vacation=='008'}">
										<tr id="vacWithoutSal" class="showClass">
												...
												id="leaveTime">
													...
													<option value="start"
														${status.value == "start" ? ' selected':''}><fmt:message
															key="requestsApproval.caption.leaveTimeStart" /></option>
													<option value="end"
														${status.value == "end" ? ' selected':''}><fmt:message
															key="requestsApproval.caption.leaveTimeEnd" /></option>
											</select></td>
											<%
											}
											...
											<td class="formBodControl"><input type="text"
												name="withdraw" id="withdraw" onfocus="calculateDiff2()"
												value="${loginUsersRequests.withdrawDays}" /></td>
											<%
											}
											%>
										</tr>
									...
									<c:if test="${loginUsersRequests.vacation.vacation!='008'}">
										<tr id="vacationDates" class="showClass">
											...
											<c:if test="${loginUsersRequests!=null}">
												<td class="formBodControl"><input type="text"
													name="withdrawDays" id="withdrawDays"
													value="${loginUsersRequests.withdrawDays}"
													onfocus="calculateDiff()" /></td>
											</c:if>
											<c:if test="${loginUsersRequests==null}">
												<td class="formBodControl"><input type="text"
													name="withdrawDays" id="withdrawDays"
									...
									<tr id="annualVacationTypes" class="hideClass">
										...
										<td class="formBodControl"><input type="text" size="10"
											...
											name="vacCredit" id="vacCredit" value="${vacCredit}" /></td>
							...
							<c:if
								test="${loginUsersRequests.request_id.id==5 || loginUsersRequests.request_id.id==3}">
							...
							<c:if test="${loginUsersRequests.request_id.id==3}">
								...
								<tr id="vacationDates" class="hideClass">
									...
									<c:if test="${loginUsersRequests==null}">
										<td class="formBodControl"><input type="text"
											name="withdrawDays" id="withdrawDays" value="${withdrawDays}"
											onfocus="calculateDiff()" /></td>
									</c:if>
								</tr>
						...
						<tr id="btn">
							<td colspan=4><abc:i18n property="commons.button.submit" /><input
								type="submit" name="save"
								value="<fmt:message key="commons.button.submit"/>"
