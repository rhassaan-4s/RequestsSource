...
function deleteMyRow(deleteId,employeeId)
{
	...
	else
	{
		 ...
		 var URL='employeeVacationIncreaseView.html?formType=${formType}&deleteId='+deleteId+'&employeeId='+employeeId;
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px ">
  ...
  <form method="post" action="<c:url value="employeeVacationIncreaseView.html?add=false"/>">
  <c:if test="${formType == 'transaction'}">
		...
		<b><font color="red"><fmt:message key="hr.errors.noEmployeeSelected"/> </font></b>
			...
			<tr>
				<td colspan="8" class="helpTitle"><abc:i18n
					property="hr.header.employeeVacationIncreaseView" /><fmt:message
					key="hr.header.employeeVacationIncreaseView" /></td>
			</tr>
			...
			<tr>
				...
				<td class="helpHed" nowrap="nowrap"> </td>
						   ...
						   <c:forEach items="${empVacationIncreaseList}" var="record">
							   <tr height=20 bgcolor="#F8F8F8">
								  ...
								  <td class="helpBod" nowrap>
								    ...
								    <td class="helpBod" nowrap>
							   <_4s_:formatMiladiDate value="${record.increaseDate }"/>      
								  ...
								  <td class="helpBod" nowrap><abc:i18n
									...
									href="employeeVacationIncreaseForm.html?empVacationIncreaseId=${record.id}&employeeId=${record.employee.id }&formType=${formType }"><fmt:message
		...
		<br>
		    ...
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeVacationIncreaseForm.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&employeeId=${employeeId}&add=true'"></input>
		    </c:if>
		    
		    <c:if test="${formType=='transaction'}">
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeVacationIncreaseView.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&add=true'"></input>
		    </c:if>	 	 
