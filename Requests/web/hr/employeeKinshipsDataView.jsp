...
function deleteMyRow(deleteId,employeeId)
{
	...
	else
	{
		 ...
		 var URL='employeeKinshipsDataView.html?formType=${formType}&deleteId='+deleteId+'&employeeId='+employeeId;
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px ">
  ...
  <form method="post" action="<c:url value="employeeKinshipsDataView.html?add=false"/>">
  <c:if test="${formType == 'transaction'}">
     ...
     <c:if test="${errorMessage!=null && errorMessage!='' }">
		<b><font color="red"><fmt:message key="hr.errors.noEmployeeSelected"/> </font></b>
			...
			<tr>
				<td colspan="8" class="helpTitle"><abc:i18n
					property="hr.header.employeeKinshipsDataView" /><fmt:message
					key="hr.header.employeeKinshipsDataView" /></td>
			</tr>
			...
			<tr>
			    ...
			    <td class="helpHed" nowrap="nowrap"><abc:i18n
					property="hr.caption.kinshipType" /> <fmt:message
					key="hr.caption.kinshipType" /></td>
					<td class="helpHed" nowrap="nowrap"><abc:i18n
					property="hr.caption.relationshipDate" /> <fmt:message
					key="hr.caption.relationshipDate" /></td>
				<td class="helpHed" nowrap="nowrap"> </td>
				<td class="helpHed" nowrap="nowrap"> </td>
				
			</tr>
						   <c:forEach items="${empRelativeList}" var="record">
							   <tr height=20 bgcolor="#F8F8F8">
								  ...
								  <td class="helpBod" nowrap>
								   ...
								   <td class="helpBod" nowrap>
								    ...
								    <td class="helpBod" nowrap>
							   <_4s_:formatMiladiDate value="${record.relationshipStartDate }"/>      
								  ...
								  <td class="helpBod" nowrap><abc:i18n
									...
									href="employeeKinshipsDataForm.html?empRelativeId=${record.id}&employeeId=${record.employee.id }&formType=${formType }"><fmt:message
		...
		<br>
		    ...
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeKinshipsDataForm.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&employeeId=${employeeId}&add=true'"></input>
		    </c:if>
		    
		    <c:if test="${formType=='transaction'}">
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeKinshipsDataView.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&add=true'"></input>
		    </c:if>	 	 
