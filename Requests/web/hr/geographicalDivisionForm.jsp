...
<tr>
		...
		<table cellspacing="0" cellpadding="0" border="0" align="right"
			...
			<table cellspacing="0" cellpadding="0" border="0" align="right">
					...
					<c:set var="i" value="geographicalDivTree" />
									...
									<tree:nodeMatch node="tree.node" hasChildren="true"
										...
										<c:choose>
											<c:when test="${result.id != null}">
												<a
													href="geographicalDivisionTree.html?expand=<tree:nodeId node="tree.node"/>&geographicalDivisionId=${result.id}"><img
										...
										<c:choose>
											...
											<c:otherwise>
												<a
													href="geographicalDivisionTree.html?collapse=<tree:nodeId node="tree.node"/>"><img
													src="/Requests/web/gl/images/ar_expandedMidNode.gif" border="0"></a>
											</c:otherwise>
										</c:choose>
									</tree:nodeMatch>
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px" align="left" >
   <form id="geographicalDivisionForm" name="geographicalDivisionForm" method="POST"	action="<c:url value="/hr/geographicalDivisionForm.html"/>">
  	...
  	<table  align="left" width="66%" class="sofT"  >
			<tr id="head_1_ep">
				<td class="helpTitle" colspan="2" nowrap><abc:i18n
					property="hr.header.addGeographicalDivision" /><fmt:message
					key="hr.header.addGeographicalDivision" /></td>
			</tr>
          ...
          <c:if test="${isLeaf != 'true'}"> 
			...
			<td class="formBodControl">
				...
				<option value=""><fmt:message key="commons.caption.select" /></option>
					<c:forEach items="${geoDivList}" var="geoDiv">
						<option value="${geoDiv.id}" ${geoDiv.id == trainingExam.trainingProgram.id ?' selected' : ''}>${geoDiv.ardesc}</option>
					</c:forEach>
				</select>
			...
			<td class="formBodControl">
				<select multiple size="6" name="leafsList" id="leafsList" >
					<c:forEach items="${leafs}" var="leaf">
						...
						<c:forEach items="${geographicalDivision.childs}" var="child">
