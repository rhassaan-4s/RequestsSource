...
function deleteMyRow(deleteId,employeeId)
{
	...
	else
	{
		 ...
		 var URL='employeeErrandView.html?formType=${formType}&deleteId='+deleteId+'&employeeId='+employeeId;
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px ">
  ...
  <form method="post" action="<c:url value="employeeErrandView.html?add=false"/>">
  <c:if test="${formType == 'transaction'}">
		...
		<b><font color="red"><fmt:message key="hr.errors.noEmployeeSelected"/> </font></b>
			...
			<tr>
				<td colspan="8" class="helpTitle"><abc:i18n
					property="hr.header.employeeErrandView" /><fmt:message
					key="hr.header.employeeErrandView" /></td>
			</tr>
			...
			<tr>
				...
				<td class="helpHed" nowrap="nowrap"> </td>
						   ...
						   <c:forEach items="${empErrandList}" var="record">
							   ...
							   <_4s_:formatMiladiDate value="${record.toDate }"/>      
								  ...
								  <td class="helpBod" nowrap><abc:i18n
									...
									href="employeeErrandForm.html?empErrandId=${record.id}&employeeId=${record.employee.id }&formType=${formType }"><fmt:message
		...
		<br>
		    ...
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeErrandForm.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&employeeId=${employeeId}&add=true'"></input>
		    </c:if>
		    
		    <c:if test="${formType=='transaction'}">
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeErrandView.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&add=true'"></input>
		    </c:if>	 	 
