...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px ">
   <form id="cityForm" name="cityForm" method="POST"	action="<c:url value="/hr/cityForm.html"/>">
   
   <input type="hidden"  id="hrCityId" name="hrCityId" value="${hrCityId }"/>
   <tr>
		<td colspan="2">
			<spring:bind path="hrCity.*">
  	...
  	<table  align="center" width="66%" class="sofT" >
			<tr id="head_1_ep">
				<td class="helpTitle" colspan="2" nowrap><abc:i18n
					property="hr.header.addCity" /><fmt:message
					key="hr.header.addCity" /></td>
			</tr>
			
		    <tr>
		  <c:set var="check"  value=""/>
		  <c:if test="${hrCityId!=null && hrCityId!='' }">
				...
				<td class="formBodControl">
					<spring:bind path="hrCity.city">
						<input size="8" maxlength="4" type="text"	name="${status.expression}" value="${status.value}" onchange="fillCode(this);"  ${check}"/> 			
					</spring:bind>
					</td>
		  ...
		  <tr>
		   <td nowrap class="formReq" width="30%">
								...
								<fmt:message key="commons.caption.country"/></td>
				<td class="formBodControl" width="70%">
					<spring:bind path="hrCity.hrCountry">
					...
					<option value=""><abc:i18n property="commons.caption.select"/><fmt:message key="commons.caption.select"/></option>
					 ...
					 <option value="${country.id }" ${country.id == hrCity.hrCountry.id ? 'selected' :'' }>
					 ${ country.name}
					  </option>
					 </c:forEach>
