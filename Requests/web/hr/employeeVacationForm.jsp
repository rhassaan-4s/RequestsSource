...
<form id="empVacation" name="empVacation" method="POST"	action="<c:url value="employeeVacationForm.html"/>" >
...
<input type="hidden"  id="empVacationId" name="empVacationId" value="${empVacationId }"/>
<tr>
		<td colspan="2">
			<spring:bind path="hrEmployeeVacation.*">
...
<tr>
    	...
    	<tr height="20">
	      ...
	      <td class="helpTitle" colspan="2">
	          <b> <fmt:message	key="hr.header.employeeVacationForm" /> </b> 
	      </td>
		</tr>
		...
		<tr>
		 ...
		 <td class="formBodControl">
		   ...
		   <spring:bind path="hrEmployeeVacation.vacation">
					...
					<option value=""><abc:i18n property="commons.caption.select"/><fmt:message key="commons.caption.select"/></option>
					 ...
					 <option value="${vacation.id }" ${vacation.id == hrEmployeeVacation.vacation.id ? 'selected' :'' }>
					 ${ vacation.name}
					  </option>
					 </c:forEach>
					...
					<tr>
					   ...
					   <b> <fmt:message	key="commons.caption.fromDate" /> </b>
					   
					   </td>
					   <td class="formBodControl">
					     <spring:bind path="hrEmployeeVacation.fromDate">
					...
					<tr>
					   ...
					   <b> <fmt:message	key="commons.caption.toDate" /> </b>
					   
					   </td>
					   <td class="formBodControl">
					     <spring:bind path="hrEmployeeVacation.toDate">
					    <input type="text" id="${status.expression }" name="${status.expression }" value="${status.value }">
					     </spring:bind>
					   </td>
					...
					<tr>
					   ...
					   <b> <fmt:message	key="hr.caption.consumed" /> </b>
					   
					   </td>
					   <td class="formBodControl">
					     <spring:bind path="hrEmployeeVacation.consumedDays">
					    <input type="text" id="${status.expression }" name="${status.expression }" value="${status.value }">
					     </spring:bind>
					   </td>
