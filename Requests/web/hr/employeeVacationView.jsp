...
function deleteMyRow(deleteId,employeeId)
{
	...
	else
	{
		 ...
		 var URL='employeeVacationView.html?formType=${formType}&deleteId='+deleteId+'&employeeId='+employeeId;
...
<table width="90%" border="0" cellspacing="0" cellpadding="0" style="padding-right:10px ">
  ...
  <form method="post" action="<c:url value="employeeVacationView.html?add=false"/>">
  <c:if test="${formType == 'transaction'}">
		...
		<b><font color="red"><fmt:message key="hr.errors.noEmployeeSelected"/> </font></b>
			...
			<tr>
				<td colspan="8" class="helpTitle"><abc:i18n
					property="hr.header.employeeVacationView" /><fmt:message
					key="hr.header.employeeVacationView" /></td>
			</tr>
			...
			<tr>
				...
				<td class="helpHed" nowrap="nowrap"> <fmt:message
					...
					property="hr.caption.vacation" /> <fmt:message
					key="hr.caption.vacation" /></td>
			    <td class="helpHed" nowrap="nowrap"><abc:i18n
					property="hr.caption.fromDate" /> <fmt:message
					key="hr.caption.fromDate" /></td>
					<td class="helpHed" nowrap="nowrap"><abc:i18n
					property="hr.caption.toDate" /> <fmt:message
					key="hr.caption.toDate" /></td>
				<td class="helpHed" nowrap="nowrap"> </td>
				<td class="helpHed" nowrap="nowrap"> </td>
				
				
			</tr>
						   <c:forEach items="${empVacationList}" var="record">
							   <tr height=20 bgcolor="#F8F8F8">
								  ...
								  <td class="helpBod" nowrap>
								    ...
								    <td class="helpBod" nowrap>
							   <_4s_:formatMiladiDate value="${record.fromDate }"/>      
							
								  </td>
								    <td class="helpBod" nowrap>
							   <_4s_:formatMiladiDate value="${record.toDate }"/>      
								  ...
								  <td class="helpBod" nowrap><abc:i18n
									...
									href="employeeVacationForm.html?empVacationId=${record.id}&employeeId=${record.employee.id }&formType=${formType }"><fmt:message
		...
		<br>
		    ...
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeVacationForm.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&employeeId=${employeeId}&add=true'"></input>
		    </c:if>
		    
		    <c:if test="${formType=='transaction'}">
		    <input type="button"  value="<fmt:message key="commons.button.add"/>" name="add" class="button"	onclick="window.location='employeeVacationView.html?employeeName=${employeeName}&employeeCode=${employeeCode }&formType=${formType }&add=true'"></input>
		    </c:if>	 	 
