...
<br>
	...
	<table rules="all" align="center" width="70%" class="sofT" >
		...
		<abc:i18n property="hr.header.coursesDivisionTree" />
		<fmt:message key="hr.header.coursesDivisionTree" />
		...
		<table cellspacing="0" cellpadding="0" border="0" width="80%">
				...
				<tr>
					...
					<abc:i18n property="hr.button.addRoot" /> <input
						...
						onclick="window.location='coursesDivisionForm.html'"></input>
						</c:if>
					</td>
				</tr>
			...
			<tr>
				<td width="70%" valign="top">
					<table cellspacing="0" cellpadding="0" border="0" width="100%">
							<c:set var="i" value="coursesDivisionTree" />
										...
										<td valign="top" >
											<tree:nodeMatch node="tree.node" hasChildren="true"
												...
												<c:choose>
													<c:when test="${result.id != null}">
														<a
															href="coursesDivisionTree.html?expand=<tree:nodeId node="tree.node"/>&isFinancial=${isFinancial}&coursesDivisionId=${result.id}"><img
															src="/Requests/web/gl/images/ar_collapsedMidNode.gif" border="0"></a>
													</c:when>
													<c:otherwise>
														<a
															href="coursesDivisionTree.html?expand=<tree:nodeId node="tree.node"/>&isFinancial=${isFinancial}"><img
															src="/Requests/web/gl/images/ar_collapsedMidNode.gif" border="0"></a>
													</c:otherwise>
												</c:choose>
											</tree:nodeMatch>
											...
											<tree:nodeMatch node="tree.node"
												...
												<c:choose>
													<c:when test="${result.id != null}">
														<a
															href="coursesDivisionTree.html?coursesDivisionId=${result.id}&isFinancial=${isFinancial}&collapse=<tree:nodeId node="tree.node"/>"><img
															src="/Requests/web/gl/images/ar_expandedMidNode.gif" border="0"></a>
													</c:when>
													<c:otherwise>
														<a
															href="coursesDivisionTree.html?collapse=<tree:nodeId node="tree.node"/>&isFinancial=${isFinancial}"><img
															src="/Requests/web/gl/images/ar_expandedMidNode.gif" border="0"></a>
													</c:otherwise>
												</c:choose>
											</tree:nodeMatch>
							...
							<td nowrap align="center">
									<c:if test="${coursesDivisionId!=null ||coursesDivisionId==''}">
										<c:choose>
											<c:when test="${result.isLast == 'true'}">
												...
												<input type="button"  
													...
													class="button"
													onclick="window.location='coursesDivisionForm.html?parentId=${result.id}&isLeaf=true'"></input>
											</c:when>
											<c:otherwise>
												...
												<input type="button"
													value="<fmt:message key="commons.button.add"/>" name="edit"
													class="button"
													onclick="window.location='coursesDivisionForm.html?parentId=${result.id}&isLeaf=false'"></input>
										...
										<input type="button" value="<fmt:message key="commons.button.edit"/>"
											...
											onclick="window.location='coursesDivisionForm.html?coursesDivisionId=${result.id}&isLeaf=${isLeaf}'"></input>

										<abc:i18n property="commons.button.delete" />
										<input
											type="button" value="<fmt:message key="commons.button.delete"/>"
											name="delete" class="button"
											onclick="window.location='coursesDivisionTree.html?deleteId=${result.id}'"></input>
								        ...
								        <c:if test="${canCopyLeafs=='true'}">
										       <abc:i18n property="hr.button.copyLeafs" />
												<input
													...
													name="delete" class="button"
													onclick="window.location='coursesDivisionForm.html?copyId=${result.id}'"></input>
										</c:if>
