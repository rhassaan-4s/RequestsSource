...
public class ValidateLoginUsersRequestForm implements Validator {
	...
	public void validateRules(LoginUsersRequests loginUsersRequests, Errors errors) {
			...
			if(loginUsersRequests.getRequest_id()!=null && loginUsersRequests.getVacation()!=null){
				...
				if(loginUsersRequests.getRequest_id().getId()==2 && loginUsersRequests.getVacation().getVacation().equals("001")){
					...
					if(loginUsersRequests.getWithdrawDays()!=null && !loginUsersRequests.getWithdrawDays().equals("")){
						...
						int difference=mgr.calculateDateDifference(loginUsersRequests.getFrom_date(),loginUsersRequests.getRequest_date());
						log.debug("--diff betn reqdate & fromdate---"+difference);
						if(period.intValue()<2){
							AnnualVacLimit limit=(AnnualVacLimit) mgr.getObjectByParameter(AnnualVacLimit.class, "vac_period", period.intValue()+"");
