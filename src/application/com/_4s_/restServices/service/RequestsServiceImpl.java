...
@Service("requestsService")//may need to rename to restService if errors occured
...
// private MySecurityDAO securityDao;
	...
	public void init() throws Exception {
		System.out.println(">>> RequestsService LOADED IN ROOT CONTEXT");
	}
...
//	 log.debug("securityManager " + securityManager);
		return securityManager.login(tenantId);
	}
	...
	public Boolean checkImei(String imei, User user) {
		...
		Imei im = securityManager.checkImei(imei, user);
		log.debug("imei " + im);
		if (im != null) {
...
//		List attendanceRequests = requestsApprovalManager.getAttendanceRequests(mCalDate.getDate(),loginUsers.getEmpCode());
			...
			restStatus = requestsApprovalManager.validateSignInOut(userRequest.getAttendanceType(), mCalDate.getDate(),
					emp);
	...
	private Map createManualAttendance(LoginUsers loginUsers, MultiCalendarDate mCalDate, Employee emp,
			AttendanceRequest userRequest) {
		...
		Double distance = requestsApprovalManager.distance(new Double(userRequest.getLatitude()),
				new Double(userRequest.getLongitude()), new Double(settings.getCompanyLat()),
				new Double(settings.getCompanyLong()));
		log.debug("lat " + new Double(userRequest.getLatitude()) + " - long " + new Double(userRequest.getLongitude()));
		log.debug("company lat " + new Double(settings.getCompanyLat()) + " - long "
				+ new Double(settings.getCompanyLong()));
		...
		try {
			String address = requestsApprovalManager.getAddressByGpsCoordinates(userRequest.getLongitude() + "",
...
//	return loginUsersRequests;
	...
	public LoginUsersRequests handleVacations(AttendanceRequest userRequest, Long empId,
			LoginUsersRequests withoutSalVac) {
		Settings settings = (Settings) requestsApprovalDAO.getObject(Settings.class, new Long(1));
		...
		Long vacCredit = requestsApprovalManager.getVacationCredit(emp.getEmpCode(), userRequest.getAttendanceType(),
				userRequest.getVacation(), mCalDateFrom.getDate());
		...
		request_id = (RequestTypes) requestsApprovalManager.getObject(RequestTypes.class,
				userRequest.getAttendanceType());
...
//			log.debug("---------loginUsersRequests.getPeriod_from()-------"+loginUsersRequests.getPeriod_from());
		...
		log.debug(
				"mCalDateFrom.getDate() " + mCalDateFrom.getDate() + " - mCalDateTo.getDate() " + mCalDateTo.getDate());
	...
	public Map userRequest(AttendanceRequest userRequest, Long empId) {
		...
		if (userRequest.getAttendanceType() == null || userRequest.getAttendanceTime() == null
				|| (userRequest.getAttendanceType().equals(new Long(1))
						&& (userRequest.getAttendanceTime2().isEmpty() || userRequest.getAttendanceTime2() == null))
				|| (userRequest.getAttendanceType().equals(new Long(2))
						&& (userRequest.getAttendanceTime2().isEmpty() || userRequest.getAttendanceTime2() == null))
				|| (userRequest.getAttendanceType().equals(new Long(3))
						&& (userRequest.getAttendanceTime2().isEmpty() || userRequest.getAttendanceTime2() == null))
				|| (userRequest.getAttendanceType().equals(new Long(4))
						&& (userRequest.getAttendanceTime2().isEmpty() || userRequest.getAttendanceTime2() == null))
				|| userRequest.getAttendanceTime().isEmpty() || userRequest.getLatitude() == null
		...
		// 5 errand
		// 7:Special Vacation-e3teyadi(1-001) Special Vacation-3arda(1-002) Special
		// Vacation-marady(1-003) Special Vacation-ra7et wardeya(1-888)
...
//			log.debug("1 reqType " + reqType);
			...
			if (loginUsersRequests != null) {
				reqType = (RequestTypes) requestsApprovalDAO.getObject(RequestTypes.class,
						userRequest.getAttendanceType());
...
//			log.debug("2 reqType " + reqType.getId());
		...
		} else if (userRequest.getAttendanceType().equals(new Long(3))) {// Permission // ||
																			// userRequest.getAttendanceType().equals(new
																			// Long(4))
...
//			log.debug("5 6 reqType " + reqType.getId());
			...
			log.debug(" reqType " + reqType.getId());
		} else if (userRequest.getAttendanceType().equals(new Long(5))
				|| userRequest.getAttendanceType().equals(new Long(6))) {// errand
//			log.debug("5 6 reqType " + reqType.getId());
		...
		if (userRequest.getAttendanceType().equals(new Long(6)) || userRequest.getAttendanceType().equals(new Long(3))
				|| userRequest.getAttendanceType().equals(new Long(5))) {// Permission End || //Full Day Permission End

			if (userRequest.getAttendanceType().equals(new Long(6))) {// userRequest.getAttendanceType().equals(new
																		// Long(4)) ||permission start and end
																		// depricated with only one request
				...
				List requests = (List) (requestsApprovalManager.checkStartedRequests(requestQueryending, emp))
						.get("Response");
...
//				requestQuery.setDateFrom(userRequest.getAttendanceTime());
				...
				log.debug("#$#$starting permissions or errands ---- checked started requests ---"
						+ startedRequestsResponse.size());
				/////////////////////// end requests
				/////////////////////// automatically////////////////////////////////////////////////
...
//				List results = (List)response.get("Response");
				...
				if (settings.getAutomaticErrandEnd() != null && !settings.getAutomaticErrandEnd().isEmpty()) {
					while (itr.hasNext()) {
						...
						log.debug("req.getRequest_id().equals(loginUsersRequests.getRequest_id() "
								+ req.getRequest_id().equals(reqType));
						if (!req.getRequest_id().getId().equals(new Long(10))
								&& !req.getRequest_id().getId().equals(new Long(11))
								&& req.getPeriod_from().before(dayBeforeEndDate.getTime()) && req.getPeriod_to() == null
								&& req.getRequest_id().equals(reqType)) {
							...
							if (req.getPeriod_from() != null) {
								log.debug(
										"request no " + req.getRequestNumber() + " started on " + req.getPeriod_from());
								...
								&& !req.getRequest_id().getId().equals(new Long(11)) && req.getPeriod_to() == null) {
							log.debug("request opened in the last 24hrs is not ended");
						} else if (req.getRequest_id().getId().equals(new Long(10))
								&& settings.getSignoutBeforePermissionErrand().equals(new Boolean(true))) {
							log.debug("sign in request opened in the last 24hrs is not ended");
							status.setCode("331");
							status.setMessage("A sign in request is already started (#"
									+ ((LoginUsersRequests) startedRequestsResponse.get(0)).getRequestNumber()
				...
				if (startedRequestsResponse.size() == 1
						&& ((LoginUsersRequests) startedRequestsResponse.get(0)).getTo_date() == null) {
					status.setCode("311");
					status.setMessage("A request is already started (#"
							+ ((LoginUsersRequests) startedRequestsResponse.get(0)).getRequestNumber()
							+ ") that hasn't been ended yet.");
					status.setStatus("False");
					response.put("Status", status);
		...
		if (automaticRequestsValidation == true) {
			if (userRequest.getAttendanceType().equals(new Long(4))) {
				...
				if (attendanceResponse != null && attendanceResponse.getSignIn() != null
					...
					status.setCode("322");
					status.setMessage(
							"Another request (" + ((LoginUsersRequests) startedRequests.get(0)).getRequestNumber()
									+ ") is made already on the specified date, full day errand is not allowed.");
					status.setStatus("False");
					response.put("Status", status);
					return response;
				}
...
//				List requests = requestsApprovalManager.getRequestsByExactDatePeriodAndEmpCode(from, t, loginUsersRequests.getEmpCode());
				...
				Map attendance = requestsApprovalManager.checkAttendance(loginUsersRequests.getPeriod_from(),
						loginUsers.getEmpCode().getEmpCode());
				...
				if (requests.size() > 0) {
					...
					while (reqItr.hasNext()) {
						...
						if (req.getPeriod_to() == null) {
							log.debug(
									"checking if overlapping requests are sign in/out " + req.getRequest_id().getId());
							if (req.getRequest_id().getId().equals(new Long(10))
									|| req.getRequest_id().getId().equals(new Long(11))) {
								...
								if (attendanceStatus.getSignIn().booleanValue() == true
										&& attendanceStatus.getSignOut().booleanValue() == false
										&& settings.getSignoutBeforePermissionErrand().equals(new Boolean(true))) {
									log.debug("request in same interval " + req.getRequestNumber()
											+ req.getPeriod_from());
									status.setCode("323");
									...
									response.put("Status", status);
									return response;
								} else if (attendanceStatus.getSignIn().booleanValue() == true
										&& attendanceStatus.getSignOut().booleanValue() == true) {
									...
									if (loginUsersRequests.getPeriod_from().after(signIn)
											&& loginUsersRequests.getPeriod_from().before(signOut)) {
										log.debug("request in same interval " + req.getRequestNumber()
												+ req.getPeriod_from());
										status.setCode("324");
								...
								status.setCode("328");
								status.setMessage("Please finish your started requests (" + req.getRequestNumber()
										+ ") during the same time interval specified");
								status.setStatus("False");
								response.put("Status", status);
								return response;
							}
							...
							if (req.getPeriod_to().compareTo(loginUsersRequests.getPeriod_from()) >= 0
									&& req.getPeriod_from().compareTo(loginUsersRequests.getPeriod_from()) <= 0) {
								if (!userRequest.getAttendanceType().equals(new Long(6))
										&& !userRequest.getAttendanceType().equals(new Long(4))) {
									status.setCode("321");
									status.setMessage(
											"The specified time interval is overlapping with one of your requests with request number "
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (!userRequest.getAttendanceType().equals(new Long(7))
				&& !userRequest.getAttendanceType().equals(new Long(8))) {
...
//	}
	...
	public Map checkStartedRequests(RequestsApprovalQuery requestQuery, Employee emp) {
		LoginUsers loggedInUser = (LoginUsers) requestsApprovalManager.getObjectByParameter(LoginUsers.class, "empCode",
				emp);
	...
	public Map checkStartedRequestsIncludingAttendance(RequestsApprovalQuery requestQuery, Employee emp) {
		LoginUsers loggedInUser = (LoginUsers) requestsApprovalManager.getObjectByParameter(LoginUsers.class, "empCode",
				emp);
		log.debug("checkStartedRequests: logged in user " + loggedInUser);
	...
	public Map getRequestsForApproval(RequestsApprovalQuery approvalQuery, List empReqTypeAccs, Employee emp) {
		LoginUsers loggedInUser = (LoginUsers) requestsApprovalManager.getObjectByParameter(LoginUsers.class, "empCode",
				emp);
		return requestsApprovalManager.getRequestsForApproval(approvalQuery.getRequestNumber(),
				approvalQuery.getEmp_code(), approvalQuery.getDateFrom(), approvalQuery.getDateTo(),
				approvalQuery.getExactDateFrom(), approvalQuery.getExactDateTo(), approvalQuery.getRequestType(),
				approvalQuery.getCodeFrom(), approvalQuery.getCodeTo(), approvalQuery.getStatusId(),
				approvalQuery.getSort(), loggedInUser, empReqTypeAccs, false, approvalQuery.getIsInsideCompany(),
				approvalQuery.getPageNumber(), approvalQuery.getPageSize());
	}

	public Map getSubmittedRequestsForApproval(RequestsApprovalQuery approvalQuery, List empReqTypeAccs, Employee emp) {
		LoginUsers loggedInUser = (LoginUsers) requestsApprovalManager.getObjectByParameter(LoginUsers.class, "empCode",
				emp);
		return requestsApprovalManager.getSubmittedRequestsForApproval(approvalQuery.getRequestNumber(),
				approvalQuery.getEmp_code(), approvalQuery.getDateFrom(), approvalQuery.getDateTo(),
				approvalQuery.getExactDateFrom(), approvalQuery.getExactDateTo(), approvalQuery.getRequestType(),
				approvalQuery.getCodeFrom(), approvalQuery.getCodeTo(), approvalQuery.getStatusId(),
				approvalQuery.getSort(), loggedInUser, empReqTypeAccs, false, approvalQuery.getIsInsideCompany(),
	...
	public Map approveRequest(RequestApproval requestApproval, Employee emp) {
		...
		Object obj = requestsApprovalManager.getObject(LoginUsersRequests.class,
				new Long(requestApproval.getRequestId()));
		log.debug("****Request to be approved/rejected " + obj);
		Map response = new HashMap();
		if (obj == null) {
			status.setCode("306");
			...
			if (requestApproval.getApprove().equals("0")) {
				...
				if (requestApproval.getModifiedDate() != null && !requestApproval.getModifiedDate().isEmpty()
						&& (modifyCal.get(Calendar.DAY_OF_MONTH) != fromCal.get(Calendar.DAY_OF_MONTH)
								|| modifyCal.get(Calendar.MONTH) != fromCal.get(Calendar.MONTH))) {
					log.debug("modifying date not allowed");
					status.setCode("381");
					...
					log.debug("requestApproval " + requestApproval);
					log.debug("loginUser " + loginUser);
					log.debug("emp " + emp);
					return requestsApprovalManager.approvalsAccessLevels(requestApproval, loginUser, emp);
				}
	...
	public Map getVacInfo(RequestApproval requestApproval) {
		...
		if (requestApproval.getRequestId() != null) {
			...
			vac = (Vacation) requestsApprovalManager.getObjectByParameter(Vacation.class, "vacation",
					requestApproval.getVac());
			log.debug("vac " + vac);
		} else {
			log.debug("error");
			status.setCode("303");
...
//			log.debug("------vacation- after--==="+vac.getVacation());
	...
	public Map getSettings() {
		...
		log.debug("settings is null? " + (settings == null));
		log.debug("company is null? " + (settings != null && settings.getCompany() == null));
	...
	public Map getPortNo(String clientName) {
		...
		try {
			client = requestsApprovalManager.getObjectByParameter(Clients.class, "clientName", clientName);
	...
	public Map getTenantId(String clientName) {
		...
		if (client == null) {
			...
			Clients c = (Clients) client;
			Map temp = new HashMap();
			temp.put("TenantId", c.getSchema());
			TenantContext.setTenant(c.getSchema());
			if (attr != null) {
				System.out.println("ServletRequestAttributes " + attr.getRequest());
				if (session != null) {
					System.out.println("%%%%%% session " + session.getId());
					session.setAttribute(KEY_TENANTID_SESSION, c.getSchema());
					System.out.println("%%%% session " + session.getAttribute("tenantId"));
				}
			}
			response.put("Response", temp);
			RestStatus status = new RestStatus();
	...
	public Map searchEmployees(EmployeeWrapper emp, Employee loggedInEmployee) {
		...
		Map map = qry.search(emp.getEmpCode(), emp.getEmpName(), "login_users", "empCode", "name", "", "1", "1", level,
				settings, emp.getPageNumber(), emp.getPageSize(), "");
		Map response = new HashMap();
...
//		} 
				...
				for (int i = 0; i < objects.size(); i++) {
					...
					log.debug("-------objects---" + ob.getDay() + "-------getTimeIn---" + ob.getTimeIn()
							+ "-------getTimeOut---" + ob.getTimeOut());
				}
...
//		log.debug("-----days 002 ---"+days.size());
	...
	public Map getAttendanceReport(RequestsApprovalQuery requestApproval, Employee emp, Settings settings) {
		...
		if (dateFrom != null && dateTo != null && !dateFrom.equals("") && !dateTo.equals("")) {
			...
			if ((empCode == null || empCode.isEmpty()) && (codeFrom == null || codeFrom.isEmpty())
					&& (codeTo == null || codeTo.isEmpty())) {
//			empReqTypeAccs = requestsApprovalManager.getEmpReqTypeAccEmpCode(emp, null);

				LoginUsers loggedInUser = (LoginUsers) requestsApprovalManager.getObjectByParameter(LoginUsers.class,
						"empCode", emp);
...
//			log.debug("access levels size " + tempLevels.size());
				...
				List objectss = requestsApprovalManager.getObjectsByParameter(AccessLevels.class, "emp_id",
						loggedInUser);
				AccessLevels lev = null;
...
//			List levs = new ArrayList();
				...
				while (itrs.hasNext()) {
					...
					if (lev != null) {
						tempLevels.addAll(
								requestsApprovalDAO.getAccessLevelsBetweenCodes(lev.getLevel_id(), empCode, empCode));
					}
					log.debug("access levels size " + tempLevels.size());
				}
...
//>>>>>>> refs/heads/Master_Final
				...
				log.debug("empArray " + empArray);
				if (empArray == null || empArray.isEmpty()) {
					empArray = "'" + emp.getEmpCode() + "'";// islam can't see his reports in getAttendanceReport
															// service
					log.debug("empArray " + empArray);
					totalObjects = requestsApprovalManager.getTimeAttendAll(empArray, fromDate, toDate, statusId,
							settings);
...
//			log.debug("-------objects---"+ob.getDay()+"-------getTimeIn---"+ob.getTimeIn()+"-------getTimeOut---"+ob.getTimeOut());
	...
	public Map editUserInfo(UserWrapper userWrapper, Employee employee) {
		...
		if (userWrapper.getEmail() != null && !userWrapper.getEmail().isEmpty()) {
			if (userWrapper.getEmail().indexOf('@') == -1 || userWrapper.getEmail().indexOf('.') == -1) {
				results.put("Results", response);
				status.setCode("316");
...
//	log.debug("pic string " +  picString.equals(userWrapper.getProfilePic()));
	...
	public Map getUserGroups(Employee employee) {
		LoginUsers loginUser = (LoginUsers) requestsApprovalDAO.getObjectByParameter(LoginUsers.class, "empCode",
				employee);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public Map insertTimesheetActivity(TimesheetActivityWrapper activity) {
		...
		if (validationStatus.getMsg().equals("Mandatory") && validationStatus.getObjAttribute().equals("name")) {
			...
			results.put("Status", status);
			return results;
		} else if (validationStatus.getMsg().equals("Mandatory")
				&& validationStatus.getObjAttribute().equals("ename")) {
			RestStatus status = new RestStatus();
			status.setCode("332");
	...
	public Map insertTimesheetPart(TimesheetPartWrapper part) {
		...
		if (validationStatus != null && validationStatus.getMsg() != null
				&& validationStatus.getObjAttribute() != null) {
			if (validationStatus.getMsg().equals("Mandatory") && validationStatus.getObjAttribute().equals("name")) {
				RestStatus status = new RestStatus();
				status.setCode("332");
				status.setMessage("Arabic Name is Mandatory");
	...
	public Map insertTimesheetSpecs(TimesheetSpecsWrapper specs) {
		...
		if (specs.getIsUsed1() == null || specs.getIsUsed1().isEmpty()) {
			...
			results.put("Status", status);
			return results;
		} else if (specs.getIsUsed1() != null && specs.getIsUsed1().equals("1")
				&& (specs.getPartName1() == null || specs.getPartName1().isEmpty())) {
			RestStatus status = new RestStatus();
			status.setCode("339");
			...
			results.put("Status", status);
			return results;
		} else if (specs.getIsUsed2() != null && specs.getIsUsed2().equals("1")
				&& (specs.getPartName2() == null || specs.getPartName2().isEmpty())) {
			RestStatus status = new RestStatus();
			status.setCode("340");
			...
			results.put("Status", status);
			return results;
		} else if (specs.getIsUsed3() != null && specs.getIsUsed3().equals("1")
				&& (specs.getPartName3() == null || specs.getPartName3().isEmpty())) {
			RestStatus status = new RestStatus();
			status.setCode("341");
		...
		results = timesheetManager.insertTimesheetSpecs(specs.getPartName1(), specs.getPartEName1(), specs.getIsUsed1(),
				specs.getPartName2(), specs.getPartEName2(), specs.getIsUsed2(), specs.getPartName3(),
				specs.getPartEName3(), specs.getIsUsed3());
		RestStatus status = new RestStatus();
		status.setCode("200");
		status.setMessage("Request Success");
		status.setStatus("True");
	...
	public Map insertTimesheetTransDefaults(TimesheetTransDefaultWrapper defaults) {
		...
		results = timesheetManager.insertTimesheetTransDefaults(defaults.getEmpCode(), defaults.getCostCenterCode(),
				defaults.getActivityCode(), defaults.getPartCode1(), defaults.getPartCode2(), defaults.getPartCode3());
		RestStatus status = new RestStatus();
		status.setCode("200");
		status.setMessage("Request Success");
		status.setStatus("True");
	...
	public Map insertTimesheetTransaction(TimesheetTransWrapper trans) {
		...
		results = timesheetManager.insertTimesheetTransaction(trans.getEmpCode(), trans.getCostCenterCode(),
				trans.getActivityCode(), trans.getInDate(), trans.getcHour(), trans.getcMinute(), trans.getPartCode1(),
				trans.getPartCode2(), trans.getPartCode3(), trans.getRemark());
	...
	public Map getParts(Short partNo) {
		...
		TimesheetTransactionParts nullPart = (TimesheetTransactionParts) timesheetManager
				.getObjectByParameter(TimesheetTransactionParts.class, "code", "9999999999");
	...
	public Map getTimesheetTransactions(TimesheetTransactionFilters search) {
		...
		if (search.getEmpCode() != null && !search.getEmpCode().isEmpty() && !search.getEmpCode().contains(",")) {
			...
			if (employee != null) {
				transMap = timesheetManager.getTimesheetTransactions(search.getEmpCode(), search.getFromDate(),
						search.getToDate(), search.getCostCenter(), search.getActivity(), search.getPart1(),
						search.getPart2(), search.getPart3(), search.getPageNumber(), search.getPageSize(),
						search.getSort());
			} else {
				RestStatus status = new RestStatus();
				status.setCode("349");
