...
public class QualificationBinder extends PropertyEditorSupport implements BaseBinder{
	...
	public String getAsText() {
		...
		Qualification qual = ((Qualification) getValue());
		String id = null;
		String text = "";
		if (qual != null) {
			id = qual.getQual();
			log.debug(">>>>>>>>>>qual.id" + id);
		}
		if (id != null) {
			text = id.toString();
			log.debug(">>>>>>>>>> text " + text);
	...
	public void setAsText(String text) throws IllegalArgumentException {
		...
		if ((text != null) && (text.length() > 0)) {
			log.debug(">>>>>>>>>>>>text " + text);
			Qualification qual = (Qualification) baseManager
					.getObjectByParameter(Qualification.class,"qual", text.replace(",", ""));
			log.debug(">>>>>>>>>>>>>>> qual " + qual);
			setValue(qual);
		}else{
			setValue(null);
