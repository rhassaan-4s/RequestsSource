...
public class LoginUsersRequestsForm extends BaseSimpleFormController {
	...
	@Autowired
	@Qualifier("validateLoginUsersRequest")
...
//	}
	...
	@RequestMapping(value = "/vacInfo", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON, consumes = MediaType.APPLICATION_FORM_URLENCODED)
	@ResponseBody
...
//		mCalDate.setDateString(newDate);
		...
		LoginUsers loginUsers = (LoginUsers) requestsApprovalManager.getObjectByParameter(LoginUsers.class, "empCode",
				emp);
		if (loginUsers != null) {
		...
		if (empRequestTypeId == null || empRequestTypeId.equals("")) {
			...
			log.debug("loginUsersRequests------" + loginUsersRequests);
			loginUsersRequests = (LoginUsersRequests) requestsApprovalManager.getObject(LoginUsersRequests.class,
					new Long(empRequestTypeId));
		}
		...
		if (settings.getVacationRequestExcep() == false) {
			if (loginUsersRequests != null && !loginUsersRequests.equals("")) {
				if (loginUsersRequests.getId() != null && !loginUsersRequests.getId().equals("")) {
					log.debug("----loginUsersRequests.getRequest_id().getId()---------"
							+ loginUsersRequests.getRequest_id().getId());
					RequestTypes specVac = (RequestTypes) requestsApprovalManager.getObject(RequestTypes.class,
							new Long(1));
...
//		Settings settings = (Settings)request.getSession().getAttribute("settings");
	...
	public Map populateWebFrameworkList(@RequestParam(value = "error", required = false) String error,
		...
		log.debug(">>>>>>>>>>>>>>>>>>>>>>> Starting populating data: >>>>>>>>>>>>>>>>>>>>>>>>>>>");
		...
		log.debug("period_from------" + period_from);
		model.put("period_from", period_from);
		MultiCalendarDate period_from_cal = new MultiCalendarDate(); 
		if (period_from != null && !period_from.isEmpty()) {
			period_from_cal.setDateTimeString(period_from);
		}
		
		String from_date = request.getParameter("from_date");
		log.debug("from_date------" + from_date);
		model.put("from_date", from_date);
		MultiCalendarDate from_cal = new MultiCalendarDate();
		if (from_date != null && !from_date.isEmpty()) {
		from_cal.setDateTimeString(from_date);
		}

		String to_date = request.getParameter("to_date");
		log.debug("to_date------" + to_date);
		model.put("to_date", to_date);
		MultiCalendarDate to_cal = new MultiCalendarDate(); 
		if (to_date != null && !to_date.isEmpty()) {
		to_cal.setDateTimeString(to_date);
		}
		
		String period_to = request.getParameter("period_to");
		log.debug("period_to------" + period_to);
		model.put("period_to", period_to);
		MultiCalendarDate period_to_cal = new MultiCalendarDate(); 
		if (period_to != null && !period_to.isEmpty()) {
		period_to_cal.setDateTimeString(period_to);
		}
		...
		String annual_vac = request.getParameter("annualVacation");
		log.debug("annual_vac------" + annual_vac);
		model.put("annual_vac", annual_vac);
		
		String spec_vac = request.getParameter("specialVacation");
		log.debug("spec_vac------" + spec_vac);
		model.put("spec_vac", spec_vac);

//		String request_date = request.getParameter("request_date");
		log.debug("request_date------" + request_date);
//		model.put("request_date", request_date);
		
		MultiCalendarDate request_date_cal = new MultiCalendarDate(); 
		if (request_date != null && !request_date.isEmpty()) {
			log.debug("if");
			log.debug(request_date);
			request_date=request_date.replace("-", "/");
			request_date_cal.setDateTimeString(request_date);
		} else {
			log.debug("else");
			request_date_cal.setDate(Calendar.getInstance().getTime());
			request_date = request_date_cal.getDateString();
		}
		model.put("request_date", request_date);
		log.debug(request_date);
		
		List orderBy = new ArrayList();
		List requestTypeList = requestsApprovalManager.getObjectsByParameterOrderedByFieldList(RequestTypes.class,
				"hidden", new Integer(0), orderBy);
		...
		if (request_id!= null && !request_id.isEmpty() && annual_vac!=null && !annual_vac.isEmpty()) {
			if (request_id.equals("2") && annual_vac.equals("001")) {
					log.debug("loginUsersRequests.getRequest_id()-==2--");
					if (withdrawDays!= null
							&& !withdrawDays.equals("")) {
						log.debug("loginUsersRequests.getWithdrawDays()-----");
						Double period = new Double(withdrawDays);
						log.debug("--period---" + period);
						int difference = requestsApprovalManager.calculateDateDifference(
								from_cal.getDate(), request_date_cal.getDate());
						log.debug("--diff betn reqdate & fromdate---" + difference);
						if (period.intValue() < 2) {
							try {
							AnnualVacLimit limit = (AnnualVacLimit) requestsApprovalManager
									.getObjectByParameter(AnnualVacLimit.class, "vac_period", period.intValue() + "");
							...
							if (limit != null && !limit.equals("")) {
								...
								if (per.equals(period.intValue() + "")) {
									if (limit.getVac_limit().equals(difference + "")
											|| Integer.parseInt(limit.getVac_limit()) < difference) {
										log.debug("----true---");
									} else {
										log.debug("----error---");
...
//								e.printStackTrace();
								log.debug("no vac limit defined for this period");
								model.put("limit", "-1");
							}

						} else if (period.intValue() >= 2) {
							AnnualVacLimit limit = (AnnualVacLimit) requestsApprovalManager
									.getObjectByParameter(AnnualVacLimit.class, "vac_period", 2 + "");
							if (limit != null && !limit.equals("")) {
		...
		if (empRequestTypeId != null && !empRequestTypeId.equals("")) {
			LoginUsersRequests logRequests = (LoginUsersRequests) requestsApprovalManager
					.getObject(LoginUsersRequests.class, new Long(empRequestTypeId));
		...
		if (done != null && reqId != null) {
			...
			if (creatingMail == true) {
				...
				fields.add("id");
				RequestTypes req = (RequestTypes) requestsApprovalManager.getObject(RequestTypes.class,
						Long.parseLong(reqId));
				List empAcc = (List) requestsApprovalManager.getObjectsByTwoParametersOrderedByFieldList(
						EmpReqTypeAcc.class, "emp_id", employee, "req_id", req, fields);
				...
				for (int j = 0; j < empAcc.size(); j++) {
					...
					mgrs = (List) requestsApprovalManager.getObjectsByParameter(AccessLevels.class, "level_id",
							empacc.getGroup_id());
					...
					for (int i = 0; i < mgrs.size(); i++) {
						...
						mm += "\n���� ����� �������� ����� ����� ������� ����� ��: \n" + loginUrl + "?requestId="
								+ reqId + "&requestNumber=" + requestNumber;
						mm += "\n�� ���� �����";
...
//						log.debug("my case 25-3 ===>>>>" + loginUsersRequests.getRequest_id().getId()
//								+ "vacation ??????????" + loginUsersRequests.getVacation().getVacation());
//						RequestTypes errands = (RequestTypes) requestsApprovalManager.getObject(RequestTypes.class,
//								new Long(4));
//						if (loginUsersRequests.getRequest_id().getId().equals(new Long(1))
//								&& loginUsersRequests.getVacation().getVacation().equals("999")) {
//							log.debug(">>>>>>>>>test=======");
////							Vacation errand=(Vacation) requestsApprovalManager.getObject(Vacation.class, "999");
//
...
//			}
	...
	public void initBinder(HttpServletRequest request, WebDataBinder binder) {
		...
		binder.registerCustomEditor(Date.class,"request_date", dateBinder);
		binder.registerCustomEditor(Date.class,"from_date", dateBinder);
		binder.registerCustomEditor(Date.class,"to_date", dateBinder);
		binder.registerCustomEditor(Date.class,"period_from", dateTimeBinder);
		binder.registerCustomEditor(Date.class,"period_to", dateTimeBinder);
		binder.registerCustomEditor(Date.class,"vac_period_from", dateTimeBinder);
		binder.registerCustomEditor(Date.class,"vac_period_to", dateTimeBinder);
...
//method=RequestMethod.POST,
	...
	public ModelAndView processSubmit(HttpServletRequest request,
			@Valid @ModelAttribute("loginUsersRequests") LoginUsersRequests command, BindingResult errors,
		...
		log.debug(loginUsersRequests.getPeriod_from());

		log.debug("notes " + loginUsersRequests.getNotes());
		try {
		...
		log.debug("notes " + loginUsersRequests.getNotes());
		log.debug(errors.getAllErrors());
		if (errors.hasErrors()) {
			model.put("done", false);
			model.put("errors", errors);
			model.put("loginUsersRequests", loginUsersRequests);
			log.debug("processSubmit  putting command object in model");
			return new ModelAndView("loginUsersRequestsForm", model);
		}
...
//		log.debug("----loginUsersRequests.getId()-onsubmit-----"+loginUsersRequests.getId()+"-----loginUsersRequests---"+loginUsersRequests.getLogin_user().getEmpCode());
		...
		log.debug("------period from command---" + loginUsersRequests.getPeriod_from()
				+ "loginUsersRequests.getPeriod_to()" + loginUsersRequests.getPeriod_to());
		...
		if (loginUsersRequests.getRequest_id() != null && !loginUsersRequests.getRequest_id().equals("")) {
			...
			if ((loginUsersRequests.getRequest_id().getId() == 4
					&& loginUsersRequests.getRequest_id().getParentId() != null
					&& !loginUsersRequests.getRequest_id().getParentId().equals(""))
					|| loginUsersRequests.getRequest_id().getId() == 5) {
...
//					Vacation vac= (Vacation)requestsApprovalManager.getObject(Vacation.class, "999");
				...
				if (accuracy != null && !accuracy.isEmpty()) {
					if (settings.getLocationAccuracy() >= Double.parseDouble(accuracy)) {
						...
						double distance = requestsApprovalManager.distance(new Double(latitude), new Double(longitude),
								new Double(settings.getCompanyLat()), new Double(settings.getCompanyLong()));
			...
			if (withoutSalaryVacEnabled == true) {// Lotus
				if (loginUsersRequests.getRequest_id().getId() == 1
						&& loginUsersRequests.getVacation().getVacation().equals("008")) {
					// tasree7 object
					log.debug("---without Salary vacation ---");
					log.debug("---------loginUsersRequests.getPeriod_from()-------"
							+ loginUsersRequests.getPeriod_from());

					LoginUsersRequests withoutSalVac = new LoginUsersRequests();
					RequestTypes request_id = (RequestTypes) requestsApprovalManager.getObject(RequestTypes.class,
							new Long(3));
		...
		//////////////////////////////////////////////////////////////
		log.debug(loginUsersRequests.getAltDate() + "," + loginUsersRequests.getApplicable() + ","
				+ loginUsersRequests.getApproved() + ",\n" + loginUsersRequests.getEmpCode() + ","
				+ loginUsersRequests.getFrom_date() + "," + loginUsersRequests.getFrom_date_history() + ","
				+ loginUsersRequests.getInputType() + ",\n" + loginUsersRequests.getIsInsideCompany() + ","
				+ loginUsersRequests.getLatitude() + "," + loginUsersRequests.getLeave_effect() + ",\n"
				+ loginUsersRequests.getLeave_type() + "," + loginUsersRequests.getLocationAddress() + ","
				+ loginUsersRequests.getLogin_user() + "," + loginUsersRequests.getLongitude() + ",\n"
				+ loginUsersRequests.getManagerModifiedDate() + "," + loginUsersRequests.getNotes() + ","
				+ loginUsersRequests.getPayed() + "," + loginUsersRequests.getPeriod_from() + ",\n"
				+ loginUsersRequests.getPeriod_to() + "," + loginUsersRequests.getPosted() + ","
				+ loginUsersRequests.getReply() + "," + loginUsersRequests.getRequestNumber() + ",\n"
				+ loginUsersRequests.getRequest_date() + "," + loginUsersRequests.getRequest_id() + ","
				+ loginUsersRequests.getTo_date() + "," + loginUsersRequests.getVacCredit() + ",\n"
				+ loginUsersRequests.getVacation() + "," + loginUsersRequests.getWithdrawDays() + ","
				+ loginUsersRequests.getId());
	...
	protected void onBind(HttpServletRequest request, Object command, BindingResult errors) throws Exception {
		...
		if (annVacation != null && !annVacation.equals("")) {
			Vacation annVac = (Vacation) requestsApprovalManager.getObjectByParameter(Vacation.class, "vacation",
					annVacation);
		...
		if (speVacation != null && !speVacation.equals("")) {
			Vacation specVac = (Vacation) requestsApprovalManager.getObjectByParameter(Vacation.class, "vacation",
					speVacation);
		...
		if (loginUsersRequests.getRequest_id() != null && !loginUsersRequests.getRequest_id().equals("")) {
			if (reqPeriodDate == true) {// Lotus
				if (loginUsersRequests.getRequest_id().getId() == 1 && loginUsersRequests.getVacation() != null
						&& loginUsersRequests.getVacation().getVacation().equals("008")) {
					if (loginUsersRequests.getVac_period_from() != null
							&& !loginUsersRequests.getVac_period_from().equals("")) {
					...
					if (loginUsersRequests.getVac_period_to() != null
							&& !loginUsersRequests.getVac_period_to().equals("")) {
				...
				if (loginUsersRequests.getRequest_id().getId() == 2
						&& loginUsersRequests.getVacation().getVacation().equals("001")) {
					if (loginUsersRequests.getVac_period_from() != null
							&& !loginUsersRequests.getVac_period_from().equals("")) {
						log.debug("----period-date from entered---" + loginUsersRequests.getVac_period_from());
		...
		log.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>> end onBind >>>>>>>>>>>>>>>>>>>>>>>>>>>");
	}

	protected void onBindAndValidate(HttpServletRequest request, Object command, BindingResult errors)
			throws Exception {
		...
		if (errors.hasErrors() == false) {
			if (loginUsersRequests.getRequest_id().getId() == 4 || loginUsersRequests.getRequest_id().getId() == 5) {
				...
				if (errors.hasErrors() == false) {
					if (longitude == null || latitude == null || Double.parseDouble(longitude) == 0
		...
		if (errors.getErrorCount() == 0) {
			if (loginUsersRequests == null || loginUsersRequests.equals("")) {
				...
				if ((loginUsersRequests.getEmpCode() != null && !loginUsersRequests.getEmpCode().equals(""))) {
					...
					Employee emp = (Employee) requestsApprovalManager.getObjectByParameter(Employee.class, "empCode",
							loginUsersRequests.getEmpCode());
					LoginUsers login_user = (LoginUsers) requestsApprovalManager.getObjectByParameter(LoginUsers.class,
							"empCode", emp);
				...
				if (loginUsersRequests.getRequest_id() != null && !loginUsersRequests.getRequest_id().equals("")) {
					if (loginUsersRequests.getRequest_id().getId() == 1
							|| loginUsersRequests.getRequest_id().getId() == 2
							|| loginUsersRequests.getRequest_id().getId() == 4) {

						// Lehaa///////////////////////////////
						if ((loginUsersRequests.getFrom_date() == null || loginUsersRequests.getFrom_date().equals(""))
								|| (loginUsersRequests.getTo_date() == null
										|| loginUsersRequests.getTo_date().equals(""))) {
							log.debug("######loginUsersRequests.getFrom_date()  " + loginUsersRequests.getFrom_date());
							log.debug("######loginUsersRequests.getTo_date()  " + loginUsersRequests.getTo_date());
						...
						if ((loginUsersRequests.getFrom_date() != null && !loginUsersRequests.getFrom_date().equals(""))
								&& (loginUsersRequests.getTo_date() != null
										&& !loginUsersRequests.getTo_date().equals(""))) {
							log.debug("from date " + loginUsersRequests.getFrom_date().getTime());
							log.debug("to date " + loginUsersRequests.getTo_date().getTime());
							Calendar temp = Calendar.getInstance();
							temp.setTime(loginUsersRequests.getFrom_date());
							temp.set(Calendar.MILLISECOND, 0);
							loginUsersRequests.setFrom_date(temp.getTime());
							temp.setTime(loginUsersRequests.getTo_date());
							temp.set(Calendar.MILLISECOND, 0);
							loginUsersRequests.setTo_date(temp.getTime());
							log.debug("from date " + loginUsersRequests.getFrom_date().getTime());
							log.debug("to date " + loginUsersRequests.getTo_date().getTime());
							log.debug("loginUsersRequests.getFrom_date().compareTo(loginUsersRequests.getTo_date()) " + loginUsersRequests.getFrom_date().compareTo(loginUsersRequests.getTo_date()));
						...
						if (loginUsersRequests.getWithdrawDays() == null
								|| loginUsersRequests.getWithdrawDays().equals("")) {
						...
						if (loginUsersRequests.getRequest_id().getId() != 4) {
							if (loginUsersRequests.getVacation() == null
									|| loginUsersRequests.getVacation().equals("")) {
						...
						if (withoutSalPeriodValidation == true && loginUsersRequests.getRequest_id().getId() == 2
							...
							if (withoutSalPeriod == null || withoutSalPeriod.equals("")) {
								...
								&& !(loginUsersRequests.getRequest_id().getId() == 1
										&& loginUsersRequests.getVacation() != null
										&& (loginUsersRequests.getVacation().getVacation().equals("008")
												|| loginUsersRequests.getVacation().getVacation().equals("010")))) // Lotus
								|| withoutSalPeriodValidation == true
										&& !(loginUsersRequests.getRequest_id().getId() == 2
												&& loginUsersRequests.getVacation().getVacation().equals("001")) // Lehaa
						) {

							if ((loginUsersRequests.getFrom_date() == null
									|| loginUsersRequests.getFrom_date().equals(""))
									|| (loginUsersRequests.getTo_date() == null
											|| loginUsersRequests.getTo_date().equals(""))) {
							...
							if ((loginUsersRequests.getFrom_date() != null
									...
									&& (loginUsersRequests.getTo_date() != null
											&& !loginUsersRequests.getTo_date().equals(""))) {
								if (loginUsersRequests.getFrom_date().after(loginUsersRequests.getTo_date())) {
									errors.rejectValue("from_date",
											"requestsApproval.errors.toTimeShouldBeGreaterThanOrEqualFromTime");
									return;
								}
							}
...
//							log.debug("loginUsersRequests.getFrom_date().before(c.getTime()) " + loginUsersRequests.getFrom_date().before(c.getTime()));
							...
							if (loginUsersRequests.getVacation() != null
									&& loginUsersRequests.getVacation().getVacation() != null
									&& loginUsersRequests.getVacation().getVacation().equals("010")) {
								if (loginUsersRequests.getAltDate() != null
										&& !loginUsersRequests.getAltDate().equals("")) {
									errors.reject("commons.errors.requiredFields");
									return;
								}
							}
							...
							log.debug("testing");
							// log.debug("--diff betn reqdate &
							// fromdate---"+calculateDifference(loginUsersRequests.getFrom_date(),
							...
							if (loginUsersRequests.getVacation() != null) {
								...
								if (loginUsersRequests.getRequest_id().getId() == 2
									...
									if (loginUsersRequests.getWithdrawDays() != null
										...
										log.debug("--period->>>>>>>--" + period);
										int difference = requestsApprovalManager.calculateDateDifference(
												loginUsersRequests.getFrom_date(),
												loginUsersRequests.getRequest_date());
										log.debug("--diff --->>>" + difference);
										if (period.intValue() < 2) {
											AnnualVacLimit limit = (AnnualVacLimit) requestsApprovalManager
													.getObjectByParameter(AnnualVacLimit.class, "vac_period",
															period.intValue() + "");
						...
						if ((loginUsersRequests.getPeriod_from() == null
								|| loginUsersRequests.getPeriod_from().equals(""))
								|| (loginUsersRequests.getPeriod_to() == null
										|| loginUsersRequests.getPeriod_to().equals(""))) {
							errors.reject("requestsApproval.errors.nullRequestPeriod");
							return;
						}
						...
						log.debug("permission from date " + loginUsersRequests.getPeriod_from() + " permission to date " + loginUsersRequests.getPeriod_to());
						...
						if (Integer.parseInt(fromHour) > Integer.parseInt(toHour)) {
							...
							return;
						} else if ((Integer.parseInt(fromHour) == Integer.parseInt(toHour))
								&& (Integer.parseInt(fromHour) != 0 && Integer.parseInt(toHour) != 0)) {
							...
							if (Integer.parseInt(fromMinutes) > Integer.parseInt(toMinutes)
									|| Integer.parseInt(fromMinutes) == Integer.parseInt(toMinutes)) {
								errors.reject("requestsApproval.errors.fromIsGreaterThanTo");
								return;
							}
						}
...
//						}
						if (loginUsersRequests.getLeave_effect() == null
								|| loginUsersRequests.getLeave_effect().equals("")) {
...
//						String perTo=loginUsersRequests.getPeriod_to()+"";
							...
							log.debug("perFrom " + perFrom);
							log.debug("perTo " + perTo);
							Map att = requestsApprovalManager.checkAttendance(loginUsersRequests.getPeriod_from(),
									loginUsersRequests.getEmpCode());
							...
							Employee e = (Employee) requestsApprovalManager.getObjectByParameter(Employee.class,
									"empCode", loginUsersRequests.getEmpCode());
							...
							if (attendanceResponse != null && attendanceResponse.getSignIn() != null
									&& attendanceResponse.getSignIn().equals(new Boolean(true))) {
								// check attendance on this day//

								System.out.println("attendance status response " + attendanceResponse.getSignIn());

								errors.rejectValue("request_id",
										"requestsApproval.errors.fullDayErrandIsNotAllowedToday",
										"User Signed In Already on the specified date, full day errand is not allowed.");
								return;
								...
								errors.rejectValue("request_id",
										"requestsApproval.errors.fullDayErrandIsNotAllowedToday",
										"Another request is made already on the specified date, full day errand is not allowed.");
								return;
							}
						} else {
							...
							List requests = requestsApprovalManager.getRequestsByExactDatePeriodAndEmpCode(
									loginUsersRequests.getPeriod_from(), loginUsersRequests.getPeriod_to(),
									loginUsersRequests.getEmpCode());
							...
							if (requests.size() > 0) {
								...
								while (reqItr.hasNext()) {
									...
									if (req.getPeriod_to() == null) {
										...
										errors.rejectValue("request_id",
												"requestsApproval.errors.endStartedRequestsInDateIntervalSpecified",
												args, "");
										return;
									} else {
										if (req.getPeriod_to().compareTo(loginUsersRequests.getPeriod_from()) > 0) {
											errors.rejectValue("request_id",
													"requestsApproval.errors.overlappingRequests");
											return;
										}
									}
								}
	...
	private void sendMessage(String smtpServer, String from, String to, String subject, String pass,
			String emailContent, String port) throws Exception {
